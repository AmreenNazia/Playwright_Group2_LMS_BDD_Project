...
When Update the fields with valid "<Scenario>" data from excel "<SheetName>"  and click save
...
Given Admin is on the Edit Class Popup window
When Update the fields with invalid "<Scenario>" data from excel "<SheetName>" and click save
Then Admin should get Error message 

Examples:
| Scenario               |   SheetName        |
|  InValidData           |   ClassPage        |

Scenario Outline: Check if the mandatory fields are updated with valid data
Given Admin is on the Edit Class Popup window
When Update the mandatory fields with valid "<Scenario>" data from excel "<SheetName>"and click save
Then Admin gets message "Class details updated Successfully " and see the updated values in data table

Examples:
| Scenario                  |   SheetName        |
|  MadatoryValid             |   ClassPage        |

Scenario Outline: Check if the optional fields are updated with valid data
Given Admin is on the Edit Class Popup window
When Update the optional fields with valid "<Scenario>" data from excel "<SheetName>" and click save
Then Admin gets message "Class details updated Successfully " and see the updated values in data table

Examples:
| Scenario                  |   SheetName        |
|  OptionalValid             |   ClassPage        |

Scenario Outline: Validate invalid values in the text fields
Given Admin is on the Edit Class Popup window
When Admin enters only numbers or special char in the text fields for "<Scenario>" data from excel "<SheetName>"
Then Admin should get Error message  Admin gets message "Class details updated Successfully " and see the updated values in data table

Examples:
| Scenario                          |   SheetName        |
|  NumericOrAlphaData               |   ClassPage        |
